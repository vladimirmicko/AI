...
public class AutoCM {
	...
	public AutoCM(String datasetFilename, int datasetDimensionY, int datasetDimensionX, double testTraingRatio) {
	...
	public AutoCM(String datasetFilename, int datasetDimensionY, int datasetDimensionX, double testTraingRatio,
			int[] numberOfLayerNodes, double learningCoeffitient) {
		...
		createTargetAA(trainingDataset);
	}
	...
	public void calculateErrors() {
		...
		for (int x = 0; x < numberOfLayerNodes[layerNumber]; x++) {
			...
			// System.out.println("Layer: "+layerNumber+", node: "+x+" Input:
			// "+nodes[0][x]+" Target: "+target[currentDatasetRow][x]+" Node
			// value: "+nodes[layerNumber][x]+" Error:
			// "+errors[layerNumber][x]);
			rowError = rowError + Math.sqrt(Math.pow(target[currentDatasetRow][x] - nodes[layerNumber][x], 2));
			// rowError=rowError+Math.pow(Math.abs(target[currentDatasetRow][x])-Math.abs(nodes[layerNumber][x]),2);
		}
		rowError = rowError / numberOfLayerNodes[layerNumber];
		// rowError=Math.sqrt(rowError);
